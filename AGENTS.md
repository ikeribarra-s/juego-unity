...
- **`DungeonGenerator.cs`** — Procedural BSP dungeon (walls + floors). `[ContextMenu]` **Generate** / **Clear** (work outside Play mode; everything parented under a single `_dungeonRoot`). **Grid-based**: BSP `Split` → rooms placed in leaves → rooms + L-corridors carved into a `bool[,] _floor` (1 tile = 1 world unit). Geometry is **derived from the grid**: floor quads via greedy horizontal run-merge; a wall is placed only on a floor tile's edge where the neighbour is empty (collinear edges merged into runs). This makes corridor↔room openings appear automatically and prevents stray walls mid-corridor — the earlier analytic approach (computing openings/clipping by hand) was abandoned for this reason. Procedural `MeshFilter`+`MeshRenderer`+`MeshCollider` per piece (so it's walkable). Default materials cached (`_runtimeFloorMat`/`_runtimeWallMat`) to avoid per-object allocation. Tunables: `Seed` (0=random), `MapWidth/Height=80`, `MinRoomSize=8`, `MaxRoomSize=20`, `MaxDepth=5`, `CorridorWidth=3`, `WallHeight=3`, `WallThickness=0.5`, optional `FloorMaterial`/`WallMaterial`.
